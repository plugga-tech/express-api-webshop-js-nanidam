...
const orderContainer = document.querySelector('.order-container') as HTMLDivElement;
const exitOrderBtn = document.querySelector('.exit-order-btn') as HTMLButtonElement;
const order = document.querySelector('.order') as HTMLDivElement;

//Login variables
const loggedinUsername = document.querySelector('.logged-in-username') as HTMLParagraphElement;
...
const loginUser = document.querySelector('.login-user-btn') as HTMLButtonElement;
const inputLoginUserMail = document.getElementById('user-email') as HTMLInputElement;
const inputLoginUserPassword = document.getElementById('user-password') as HTMLInputElement;

//Register variables
const registerUser = document.querySelector('.user-register') as HTMLLinkElement;
const registerPage = document.querySelector('.register-page') as HTMLDivElement;
const registerUserBtn = document.querySelector('.register-user-btn') as HTMLButtonElement;
const inputRegisterUsername = document.getElementById('register-username') as HTMLInputElement;
const inputRegisterUserMail = document.getElementById('register-user-email') as HTMLInputElement;
const inputRegisterUserPassword = document.getElementById('register-user-password') as HTMLInputElement;

const allCategories = document.querySelector('.header') as HTMLHeadElement;
const productPrice = document.querySelector('.product-price') as HTMLParagraphElement;
...
const cartCancelBtn = document.querySelector('.cart-cancel-btn') as HTMLButtonElement;
const cartSendBtn = document.querySelector('.cart-send-btn') as HTMLButtonElement;
const cartTotalSum = document.querySelector('.cart-total-sum') as HTMLParagraphElement;
...
const plusBtn = () => {
  const addProductBtns = document.querySelectorAll('.product-increase-btn') as NodeListOf<HTMLButtonElement>;

  addProductBtns.forEach((btn: HTMLButtonElement) => {
    btn.addEventListener('click', (e: MouseEvent) => {
      const getProductName = (e.currentTarget as HTMLElement).parentElement?.nextElementSibling?.childNodes[1]
        .childNodes[0] as HTMLParagraphElement;
      const productName = getProductName.innerHTML;

      const getProductPrice = (e.currentTarget as HTMLElement).parentElement?.nextElementSibling?.childNodes[3]
        .childNodes[0] as HTMLParagraphElement;
      const productPrice = getProductPrice.innerHTML;

      const productCount = (e.currentTarget as HTMLElement).parentElement?.childNodes[3]
      ...
      const productIndex = newCart.findIndex((prod: any) => prod.name === productName);
...
const minusBtn = () => {
  const subtractProductBtns = document.querySelectorAll('.product-decrease-btn') as NodeListOf<HTMLButtonElement>;
...
const productsCategory = async () => {
  ...
  const allCategories = document.querySelectorAll('.category') as NodeListOf<HTMLButtonElement>;
  ...
  allCategories.forEach((categoryBtn: HTMLButtonElement) => {
    categoryBtn.addEventListener('click', async (e: MouseEvent) => {
      ...
      const category = categoriesId.find((cat) => cat.name === (categoryText as string));
      ...
      const configCategory = {
        ...
        method: 'GET',
        url: category ? `http://localhost:3000/api/products/category/${category._id}` : '',
      };
...
const renderCart = (cart: any) => {
  ...
  cart.forEach((product: { count: number; name: string; price: number }) => {
    ...
    <div class="cart-product">
      ...
      <div class="cart-product-info">
        ...
        <p class="">Summa: <span class="cart-product-sum">${product.count * product.price}</span> kr</p>
      </div>
    </div>    
    `;
  });
  ...
  const totalSum = cart.reduce(
    (acc: number, product: { count: number; name: string; price: number }) => acc + product.count * product.price,
    0
  );
...
cartSendBtn.addEventListener('click', async () => {
  const foundLoggedinUser = JSON.parse(localStorage.getItem('loggedInUser') || '[]');
  ...
  if (currentCart.length) {
    currentCart.forEach((product: { name: string; count: number; price: string; id: string }) => {
